...
export default function ChatPage(){
  ...
  const send = (e) => {
    ...
    setTimeout(() => {
      setMessages(prev => [
        ...prev,
        { role: 'assistant', content: `Thanks for sharing. Let's break this down step by step.` },
      ]);
    }, 300);
  };
  ...
  return (
    <div className="flex flex-col h-full">
      <div className="mb-4">
        ...
        <p className="text-gray-600">{persona.field} • {persona.tagline}</p>
      </div>
      ...
      <div className="flex-1 overflow-y-auto bg-white border rounded-lg p-4 space-y-3">
        ...
        {messages.map((m, i) => (
          <div key={i} className={m.role === 'user' ? 'text-right' : 'text-left'}>
            <div className={`inline-block px-3 py-2 rounded-lg ${m.role === 'user' ? 'bg-primary text-white' : 'bg-gray-100 text-gray-800'}`}>
              {m.content}
            </div>
          </div>
        ))}
