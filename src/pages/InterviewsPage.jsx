...
export default function InterviewsPage(){
  ...
  useEffect(() => {
    const onStorage = (e) => { if (e.key === STORE_KEY) setSessions(loadAll()); };
  ...
  return (
    <div>
      ...
      <p className="text-gray-600 mb-8">Sharpen your skills and track past practice sessions.</p>
      ...
      <div className="grid md:grid-cols-3 gap-6 mb-10">
        {types.map(t=>(
          <div key={t.id} className="bg-white p-6 rounded-lg shadow-md border">
            ...
            <button onClick={()=>start(t.id)} className="bg-primary text-white font-semibold px-4 py-2 rounded-lg">
              Start
            </button>
          </div>
        ))}
      ...
      {sessions.length === 0 ? (
        ...
        <div className="space-y-2">
          {sessions.map(s=>(
            <div key={s.id} className="flex items-center justify-between bg-white border rounded-md p-3">
              <div className="min-w-0">
                <div className="font-semibold capitalize truncate">{s.type}</div>
                <div className="text-sm text-gray-600">
                  {s.completedAt ? 'Completed' : 'In progress'} • {new Date(s.startedAt).toLocaleString()}
                </div>
              </div>
              <div className="flex items-center gap-3">
                <Link className="text-primary font-semibold" to={`/interview?type=${s.type}`}>Open</Link>
                <button
