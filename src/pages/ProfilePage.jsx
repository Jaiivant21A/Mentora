    ...
    const getProfile = async () => {
      try {
        ...
        const { data: profileData, error: profileError } = await supabase
          .from('profiles')
          .select(`username, full_name, avatar_url, age, headline, experience_level, gender`)
          .eq('id', user.id)
          .single();
        ...
        if (profileData) {
          ...
          setAge(profileData.age !== null ? profileData.age.toString() : '');
            ...
            <form onSubmit={handleUpdateProfile} className="space-y-5">
              ...
              <div>
                ...
                <input
                  ...
                  className="mt-1 block w-full px-3 py-2 bg-background border border-border rounded-md shadow-sm focus:outline-none text-text-base"
                />
              </div>
              <div>
                <label className="block text-sm font-medium text-text-secondary">Headline</label>
                <select
                  value={headline}
                  onChange={e => setHeadline(e.target.value)}
                  className="mt-1 block w-full px-3 py-2 bg-background border border-border rounded-md shadow-sm text-text-base"
                >
              ...
              <div>
                <label className="block text-sm font-medium text-text-secondary">Experience Level</label>
                <select
