...
// (Helper functions like loadPrefs, savePrefs, formatDBDate remain the same)
...
export default function GoalsPage() {
  ...
  // QoL Improvement: State to track when a form is being submitted.
  ...
  // QoL Improvement: Auto-hide success message after 3 seconds.
  useEffect(() => {
    if (lastMsg) {
  ...
  // Memoized list for filtering and sorting
    ...
    <div>
      <div className="flex flex-col gap-2 mb-6 md:flex-row md:items-end md:justify-between">
        <div>
          ...
          <p className="text-gray-600 dark:text-gray-300">Create SMART goals, track progress, and keep deadlines in sight.</p>
        </div>
        <div className="flex gap-2">
          <div className="relative">
            <ArrowUpDown className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400" size={18} />
            <select className="pl-9 pr-3 py-2 border rounded-md" value={prefs.sortBy} onChange={(e) => setPrefs(p => ({ ...p, sortBy: e.target.value }))}>
          ...
          <label className="flex items-center gap-2 border rounded-md px-3">
            ...
            <span className="text-sm text-gray-700 dark:text-gray-200">Show completed</span>
          </label>
        </div>
      </div>
      ...
      <div className="bg-white dark:bg-gray-800 border dark:border-gray-700 rounded-lg p-4 mb-6" onKeyDown={onFormKeyDown}>
        <div className="grid gap-3 md:grid-cols-4">
            <div className="md:col-span-2">
                <label className="block text-sm text-gray-600 dark:text-gray-300 mb-1" htmlFor="goal-title">Title</label>
                <input id="goal-title" className="w-full border rounded-md px-3 py-2 dark:bg-gray-900" placeholder="e.g., Finish React course" value={form.title} onChange={(e) => setForm((f) => ({ ...f, title: e.target.value }))} required />
            </div>
            <div>
                <label className="block text-sm text-gray-600 dark:text-gray-300 mb-1 flex items-center gap-1" htmlFor="goal-progress"><Percent size={16} /> Progress (0–100)</label>
                <input id="goal-progress" type="number" min={0} max={100} className="w-full border rounded-md px-3 py-2 dark:bg-gray-900" value={form.progress} onChange={(e) => setForm((f) => ({ ...f, progress: e.target.value }))} required />
            </div>
            <div>
                <label className="block text-sm text-gray-600 dark:text-gray-300 mb-1 flex items-center gap-1" htmlFor="goal-deadline"><CalendarIcon size={16} /> Deadline</label>
                <input id="goal-deadline" type="date" className="w-full border rounded-md px-3 py-2 dark:bg-gray-900" value={form.deadline} onChange={(e) => setForm((f) => ({ ...f, deadline: e.target.value }))} />
            </div>
        </div>
        <div className="mt-4 flex gap-3">
            {editingId ? (
                <>
                    ...
                    <button onClick={cancelEdit} className="flex items-center gap-2 bg-gray-200 dark:bg-gray-700 text-gray-800 dark:text-gray-200 px-4 py-2 rounded-md"><X size={18} /> Cancel</button>
                </>
            ) : (
            ...
            <input className="ml-auto border rounded-md px-3 py-2 w-full md:w-64 dark:bg-gray-900" placeholder="Search goals..." value={query} onChange={(e) => setQuery(e.target.value)} />
        </div>
      </div>

      <div className="space-y-4">
        {filteredSorted.map((g) => (
          <div key={g.id} className="bg-white dark:bg-gray-800 border dark:border-gray-700 rounded-lg p-3 flex items-center gap-3">
            <div className="flex-1">
              {/* This component is assumed to exist and correctly display a goal */}
              <GoalTracker goal={g} />
            </div>
            <div className="flex gap-2">
                {g.status !== "Completed" && <button onClick={() => markComplete(g.id)} className="p-2 rounded-md border dark:border-gray-600 hover:bg-gray-100 dark:hover:bg-gray-700" title="Mark complete"><CheckCircle2 size={18} /></button>}
                <button onClick={() => duplicateGoal(g)} className="p-2 rounded-md border dark:border-gray-600 hover:bg-gray-100 dark:hover:bg-gray-700" title="Duplicate"><Copy size={18} /></button>
                <button onClick={() => startEdit(g)} className="p-2 rounded-md border dark:border-gray-600 hover:bg-gray-100 dark:hover:bg-gray-700" title="Edit"><Pencil size={18} /></button>
                <button onClick={() => removeGoal(g.id)} className="p-2 rounded-md border dark:border-gray-600 hover:bg-gray-100 dark:hover:bg-gray-700" title="Delete"><Trash2 size={18} /></button>
            </div>
          </div>
        ))}
        {filteredSorted.length === 0 && <p className="text-gray-500 dark:text-gray-400 text-center py-4">No goals match your filters.</p>}
      </div>
    </div>
  );
}