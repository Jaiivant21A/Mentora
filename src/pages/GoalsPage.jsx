...
import {
  ...
  Calendar as CalendarIcon, Percent, ArrowUpDown,
  CheckCircle2, Copy
} from 'lucide-react';
...
const loadPrefs = () => {
  try {
    ...
    return raw ? JSON.parse(raw) : { sortBy: 'updatedAt_desc', showCompleted: true };
  } catch {
...
export default function GoalsPage(){
  ...
  useEffect(()=>{ saveGoals(goals); },[goals]); // Write-through persistence is the safest approach with localStorage [web:78]
  useEffect(()=>{ savePrefs({ sortBy, showCompleted }); },[sortBy, showCompleted]); // Keep UI preferences durable between visits [web:78]
  ...
  const filteredSorted = useMemo(()=>{
    ...
    switch (sortBy) {
      case 'deadline_asc': return [...list].sort((a,b)=> byDeadline(a.deadline)-byDeadline(b.deadline));
  ...
  const resetForm = ()=> { setForm(emptyForm); setErrorMsg(''); };

  // CRUD
  const addGoal = ()=>{
    const err = validate({ requireDeadline:true });
    if (err) { setErrorMsg(err); setLastMsg(''); return; }
    const g = {
  ...
  const startEdit = (id)=>{
    ...
    if (!g) return;
    setEditingId(id);
    setForm({ title: g.title, progress: g.progress, deadline: ddmmyyToISO(g.deadline) }); // Prefill from dd/mm/yy to ISO for date input [web:116]
    setErrorMsg(''); setLastMsg('');
  }; // Users see the current deadline and don’t need to reselect unless changing it, which matches expected CRUD UX [web:76]

  const saveEdit = ()=>{
    const p = Number(form.progress);
    const err = !form.title.trim() ? 'Title is required' : (Number.isNaN(p) || p<0 || p>100 ? 'Progress must be 0–100' : '');
    if (err) { setErrorMsg(err); setLastMsg(''); return; }

    setGoals(prev=> prev.map(g=>{
      if (g.id!==editingId) return g;
      const newDeadline = form.deadline ? formatDDMMYY(form.deadline) : g.deadline; // Keep existing deadline unless user picked a new one [web:116]
      return { ...g, title: form.title.trim(), progress: Number(form.progress), deadline: newDeadline, updatedAt: Date.now() };
    }));
    setEditingId(null);
  ...
  const cancelEdit = ()=> { setEditingId(null); setLastMsg('Edit canceled'); resetForm(); }; // Cancel editing is an expected form affordance [web:76]
  ...
  const markComplete = (id)=> {
    setGoals(prev=> prev.map(g=> g.id===id ? { ...g, progress: 100, updatedAt: Date.now() } : g));
  ...
  const duplicateGoal = (id)=> {
    const g = goals.find(x=> x.id===id);
    if (!g) return;
    const copy = { ...g, id: crypto.randomUUID(), title: `${g.title} (copy)`, createdAt: Date.now(), updatedAt: Date.now() };
    ...
    <div>
      ...
      <div className="flex flex-col gap-2 mb-6 md:flex-row md:items-end md:justify-between">
        <div>
          ...
          <p className="text-gray-600">Create SMART goals, track progress, and keep deadlines in sight. Saved locally in this browser.</p>
        </div>
      ...
      <p className="sr-only" aria-live="polite">{errorMsg}</p>
      <p className="sr-only" aria-live="polite">{lastMsg}</p> {/* Announce success actions non-visually as well [web:94] */}

      {/* Add / Edit Form */}
      <div className="bg-white border rounded-lg p-4 mb-6" onKeyDown={onFormKeyDown}>
        <div className="grid gap-3 md:grid-cols-4">
          <div className="md:col-span-2">
            <label className="block text-sm text-gray-600 mb-1" htmlFor="goal-title">Title</label>
            <input
              id="goal-title"
          ...
          <div>
            <label className="block text-sm text-gray-600 mb-1 flex items-center gap-1" htmlFor="goal-progress">
            ...
            <input
              ...
              aria-invalid={Boolean(errorMsg && (Number.isNaN(Number(form.progress)) || form.progress<0 || form.progress>100))}
              required
            />
          </div>
          <div>
            <label className="block text-sm text-gray-600 mb-1 flex items-center gap-1" htmlFor="goal-deadline">
            ...
            />
            <p className="text-xs text-gray-500 mt-1">Tip: Ctrl/Cmd+Enter to {editingId ? 'save' : 'add'}, Esc to cancel edit.</p>
          </div>
        </div>
        ...
        <div className="mt-4 flex gap-3">
          {editingId ? (
            <>
              <button onClick={saveEdit} className="flex items-center gap-2 bg-primary text-white px-4 py-2 rounded-md">
                <Save size={18} /> Save
              </button>
              <button onClick={cancelEdit} className="flex items-center gap-2 bg-gray-200 text-gray-800 px-4 py-2 rounded-md">
                <X size={18} /> Cancel
              </button>
            </>
          ) : (
            <button onClick={addGoal} className="flex items-center gap-2 bg-primary text-white px-4 py-2 rounded-md">
        ...
        {errorMsg && <p className="mt-3 text-sm text-red-600" role="alert">{errorMsg}</p>}
      </div>

      {/* Goal list */}
      <div className="space-y-4">
        {filteredSorted.length > 0 ? filteredSorted.map(g=>(
          <div key={g.id} className="flex items-center gap-3">
            ...
            <div className="flex gap-2">
              {g.progress < 100 && (
                <button onClick={()=>markComplete(g.id)} className="p-2 rounded-md border hover:bg-gray-100" title="Mark complete">
                  <CheckCircle2 size={18} />
                </button>
              )}
              <button onClick={()=>duplicateGoal(g.id)} className="p-2 rounded-md border hover:bg-gray-100" title="Duplicate">
                <Copy size={18} />
              </button>
              <button onClick={()=>startEdit(g.id)} className="p-2 rounded-md border hover:bg-gray-100" title="Edit">
                <Pencil size={18} />
              </button>
              <button onClick={()=>removeGoal(g.id)} className="p-2 rounded-md border hover:bg-gray-100" title="Delete">
                <Trash2 size={18} />
              </button>
            </div>
          </div>
